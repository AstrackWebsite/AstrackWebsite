  ...
  const { error } = await supabase.from("project_closeout").upsert(
    {
      ...
      fb_sufficient_time: String(formData.get("fb_sufficient_time") ?? "").trim() || null,
      fb_enough_persons: String(formData.get("fb_enough_persons") ?? "").trim() || null,
      fb_enough_materials: String(formData.get("fb_enough_materials") ?? "").trim() || null,
      fb_changes_made: String(formData.get("fb_changes_made") ?? "").trim() || null,
      fb_improvements: String(formData.get("fb_improvements") ?? "").trim() || null,
    },
